...
- PlayStation Portable (text rendering issues)
...
If you're on an Arch-based Linux distribution, you can use the [`psp-sdk-bin`](https://aur.archlinux.org/packages/psp-sdk-bin/) AUR package to install PSPDEV alongside the necessary SDL2 libraries. For other dependencies, you can run `pacman -S cmake git imagemagick make python python-cairosvg`.
...
This will produce a `PSP/GAME` directory with the necessary files in the `build/<kula|kula3D|kulatwo>/` directory. You can also run `make` to build all three targets at once.
