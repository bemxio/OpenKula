...
If you're on an Arch-based Linux distribution, you can use the [`vitasdk-bin`](https://aur.archlinux.org/packages/vitasdk-bin/) and [`vitasdk-packages-bin`](https://aur.archlinux.org/packages/vitasdk-packages-bin/) AUR packages to install Vita SDK alongside the necessary SDL2 libraries.
...
This will produce a VPK package file inside the `build/<kula|kula3D|kulatwo>` directory. You can also run `make` to build all three targets at once.
